...
export async function InsertOrUpdateCompetitions() {
    const API_URL =
      "https://rest.entitysport.com/exchange/competitions?token=7b58d13da34a07b0a047e129874fdbf4&per_page=100";
...
export async function InsertOrUpdateLiveMatches() {
  const API_URL =
...
export async function InsertOrUpdateUpcomingMatches() {
  const API_URL =
    "https://rest.entitysport.com/exchange/matches?token=7b58d13da34a07b0a047e129874fdbf4&status=1&per_page=100";
...
export async function InsertOrUpdateCompletedMatches() {
  const API_URL =
    "https://rest.entitysport.com/exchange/matches?token=7b58d13da34a07b0a047e129874fdbf4&status=2&per_page=100";
...
export async function InsertOrUpdateMatchesSqauds_NotUSE() {
    try {
        const matchQuery = `SELECT match_id FROM matches m WHERE NOT EXISTS ( SELECT 1 FROM match_playing11 mp WHERE mp.match_id = m.match_id ) limit 1000`; // Adjust table name if needed
        const connection = await db.getConnection();
        ...
        console.log(`Fetching data for ${matchResults.length} matches...`);
          ...
          limit(async () => {
            try {
              ...
              return {
                ...
                teamaId: data?.response?.["match-playing11"]?.teama?.team_id || 0,
                teambId: data?.response?.["match-playing11"]?.teamb?.team_id || 0,
                teamaSquad: JSON.stringify(data?.response?.["match-playing11"]?.teama?.squads || {}).replace(/'/g, "\\'"),
                teambSquad: JSON.stringify(data?.response?.["match-playing11"]?.teamb?.squads || {}).replace(/'/g, "\\'"),
              };
            } catch (error) {
        ...
        const matchData = await Promise.all(apiCalls);
        const filteredMatches = matchData.filter((match) => match !== null);
    
        if (filteredMatches.length === 0) {
          console.log("No valid match data retrieved.");
          return;
        }
    
        console.log(`Inserting ${filteredMatches.length} matches into database...`);
    
        // ✅ Batch Insert for Performance
        const values = filteredMatches.map((m) => `(${m.match_id}, '${m.teamaId}', '${m.teambId}', '${m.teamaSquad}', '${m.teambSquad}')`).join(", ");
        const query = `
          INSERT INTO match_playing11 (match_id, teamaId, teambId, teamaSquad, teambSquad)
          VALUES ${values}
          ON DUPLICATE KEY UPDATE 
              teamaId = VALUES(teamaId),
              teambId = VALUES(teambId),
              teamaSquad = VALUES(teamaSquad),
              teambSquad = VALUES(teambSquad);
        `;
...
export async function UpdateManOfTheMatches() {
    try {
        const matchQuery = `SELECT match_id FROM matches WHERE man_of_the_match_pid IS NULL AND status = 2 LIMIT 1000`; 
        const connection = await db.getConnection();
        ...
        const apiCalls = matchResults.map((row: { match_id: any }) =>
            limit(async () => {
                try {
                    ...
                    // // ✅ Exclude entries where pid is 0 or missing
                    // if (!data?.response?.man_of_the_match?.pid) {
                    //     console.log(`Skipping match_id ${row.match_id} due to invalid pid`);
                    //     return null;
                    // }

                    return {
                        match_id: row.match_id,
                        pid: data.response.man_of_the_match.pid || 0,
                        name: data.response.man_of_the_match.name || '',
                    };
        ...
        console.log(`Updating ${filteredMatches.length} matches in the database...`);

        const matchIds = filteredMatches.map((m) => m.match_id).join(", ");
        // ✅ Batch Update Query
        const updateQuery = `
    UPDATE matches
    SET 
        man_of_the_match_pid = CASE 
            ${filteredMatches.map(m => `WHEN match_id = ${m.match_id} THEN '${m.pid}'`).join(" ")}
        END,
        man_of_the_match_pname = CASE 
            ${filteredMatches.map(m => `WHEN match_id = ${m.match_id} THEN '${m.name.replace(/'/g, "''")}'`).join(" ")}
        END
    WHERE match_id IN (${matchIds});
`;

        await connection.query(updateQuery);
        console.log("Data successfully updated!",updateQuery);
        connection.release();
...
export async function GetMatches() {
  const query = "SELECT * FROM matches ORDER BY id DESC limit 10 OFFSET 1";
  const connection = await db.getConnection();
...
export async function MatchInfo() {
    try {
        // const matchQuery = `SELECT match_id FROM matches WHERE match_id = 51451`; 
  ...
  export async function CompetitionInfo() {
    try {
        const competitionsQuery = `SELECT cid FROM competitions`; 
        const connection = await db.getConnection();
        const [competitionsResults]: any = await connection.query(competitionsQuery);
        ...
        const apiCalls = competitionsResults.map((row: { cid: any }) =>
            limit(async () => {
                try {
                   ...
                   const cid = row.cid;
                    // Define absolute file path on Windows
                        const storageDir = '/var/tmp/ucc/CompetitionData';
                        if (!fs.existsSync(storageDir)) fs.mkdirSync(storageDir, { recursive: true });
        ...
        const apiCalls = matchResults.map((row: { match_id: any }) =>
            limit(async () => {
                try {
                   ...
                   const matchId = row.match_id;
                    // Define absolute file path on Windows
                        const storageDir = '/var/tmp/ucc/MatchStatisticsData';
                        if (!fs.existsSync(storageDir)) fs.mkdirSync(storageDir, { recursive: true });
            ...
            limit(async () => {
                try {
                  for (const stat of stats) {
                    ...
                    const cid = row.cid;
                    // Define absolute file path on Windows
                        const storageDir = '/var/tmp/ucc/CompetitionStatsData';
                        if (!fs.existsSync(storageDir)) fs.mkdirSync(storageDir, { recursive: true });

                        const filePath = path.join(storageDir, `competition_${statType}_${cid}.json`);
        ...
        const apiCalls = competitionsResults.map((row: { cid: any }) =>
            limit(async () => {
                try {
                  ...
                  const API_URL =
                    `https://rest.entitysport.com/exchange/competitions/${row.cid}/matches?token=7b58d13da34a07b0a047e129874fdbf4&per_page=100`;
                  ...
                  const serieses = matches;
                   const cid = row.cid; 
                    // Define absolute file path on Windows  
                        const storageDir = '/var/tmp/ucc/CompetitionMatchesData';
                        if (!fs.existsSync(storageDir)) fs.mkdirSync(storageDir, { recursive: true });

                        const filePath = path.join(storageDir, `competition_match_${cid}.json`);
                        fs.writeFileSync(filePath, JSON.stringify(serieses, null, 2));
  ...
  export async function MatchCommentary() {
    ...
    try {
      const matchQuery = `SELECT match_id, latest_inning_number FROM matches WHERE commentary = 1`;
      const connection = await db.getConnection();
      const [matchResults]: any = await connection.query(matchQuery);
      ...
      const apiCalls = matchResults.flatMap((match: { match_id: any; latest_inning_number: any; }) => {
        const { match_id, latest_inning_number } = match;
        
        // Generate an array of innings from 1 to latest_inning_number
        return Array.from({ length: latest_inning_number }, (_, i) => i + 1).map((inningNumber) =>
          limit(async () => {
            try {
              ...
              if (!commentaryData) {
                console.log(`No commentary data for match_id ${match_id}, inning ${inningNumber}`);
                return;
              }
  
              // Define file path
              const filePath = path.join(STORAGE_DIR, `match_commentary_${inningNumber}inning_${match_id}.json`);
              fs.writeFileSync(filePath, JSON.stringify(commentaryData, null, 2));
              ...
              `;
              await connection.query(insertQuery, [match_id, inningNumber, formattedFilePath, formattedFilePath]);
  
              // console.log(`Commentary saved for match ${match_id}, inning ${inningNumber}`);
              connection.release();
            } catch (error) {
              console.error(`Failed to fetch commentary for match_id ${match_id}, inning ${inningNumber}:`, error);
            }
          })
        );
      });
  ...
  export async function TeamPlayersData() {
    try {
        ...
        const connection = await db.getConnection();
        const [teamResults]: any = await connection.query(teamQuery);
        ...
        const apiCalls = teamResults.map((row: { tid: any }) =>
            limit(async () => {
                try {
                   ...
                   const tid = row.tid;
                    // Define absolute file path on Windows
                        const storageDir = '/var/tmp/ucc/TeamPlayerData';
                        if (!fs.existsSync(storageDir)) fs.mkdirSync(storageDir, { recursive: true });
  ...
  export async function PlayerStatsData() {
    try {
        ...
        const connection = await db.getConnection();
        const [playerResults]: any = await connection.query(playerQuery);
        ...
        const apiCalls = playerResults.map((row: { pid: any }) =>
            limit(async () => {
                try {
                    ...
                    const pid = row.pid;
                    // Define absolute file path on Windows
                        const storageDir = '/var/tmp/ucc/PlayerStatsData';
                        if (!fs.existsSync(storageDir)) fs.mkdirSync(storageDir, { recursive: true });
        ...
        const apiCalls = playerResults.map((row: { pid: any }) =>
            limit(async () => {
                try {
                    ...
                    const pid = row.pid;
                    // Define absolute file path on Windows
                        const storageDir = '/var/tmp/ucc/PlayerAdvanceStatsData';
                        if (!fs.existsSync(storageDir)) fs.mkdirSync(storageDir, { recursive: true });

                        const filePath = path.join(storageDir, `player_advance_stats_${pid}.json`);
                        fs.writeFileSync(filePath, JSON.stringify(players, null, 2));
        ...
        const apiCalls = matchResults.map((row: { match_id: any }) =>
            limit(async () => {
                try {
                    ...
                    const matchId = row.match_id;
                    // Define absolute file path on Windows
                        const storageDir = '/var/tmp/ucc/MatchAdvanceData';
                        if (!fs.existsSync(storageDir)) fs.mkdirSync(storageDir, { recursive: true });
  ...
  export async function TeamMatches() {
    try {
        ...
        const connection = await db.getConnection();
        const [teamsResults]: any = await connection.query(teamsQuery);
        ...
        const apiCalls = teamsResults.map((row: { tid: any }) =>
            limit(async () => {
                try {
                  ...
                  const API_URL =
                    `https://rest.entitysport.com/exchange/teams/${row.tid}/matches?token=7b58d13da34a07b0a047e129874fdbf4&per_page=100`;
                  ...
                  if (totalPages > 1) {
                      ...
                      results.forEach(pagedata => {
                          const responseData = Array.isArray(pagedata?.response) ? pagedata.response : [];
                  ...
                  const serieses = matches;
                   const tid = row.tid; 
                    // Define absolute file path on Windows  
                        const storageDir = '/var/tmp/ucc/TeamMatchesData';
                        if (!fs.existsSync(storageDir)) fs.mkdirSync(storageDir, { recursive: true });
  ...
  export async function readMatchData() {
    const matchId  = 87016;
    const [rows]: [any[], any]  = await db.execute('SELECT * FROM match_info WHERE match_id = ? LIMIT 1', [matchId]);
    const filePath =   rows[0].fileName;
    // console.log("test", rows[0].fileName);
    try {
        // const filePath = `/var/tmp/ucc/MatchData\\match_${matchId}.json`;
        ...
        console.log(`reading match data:`, jsonData);
...
export async function deleteFilesByMatchId() {
  try {
    
    // Fetch file paths from the database
    const query = `SELECT fileName FROM match_commentary WHERE match_id in (SELECT match_id FROM matches WHERE commentary = 0)`;
    const [files]:  any = '';//await db.query(query);

    if (!files.length) {
      console.log(`No files found for match_id.`);
      return;
    }
    ...
    for (const file of files) {
      ...
      const filePath = path.resolve(file.fileName); // Convert to absolute path
