...
export async function GET(req: NextRequest) {
  try {
    ...
    const query =
      "SELECT mi.fileName FROM match_info mi JOIN ( SELECT match_id FROM matches WHERE JSON_UNQUOTE(JSON_EXTRACT(`competition`, '$.cid')) in (SELECT `cid` FROM `competitions` WHERE `is_featured` = 1) and DATE(date_start_ist) BETWEEN DATE(NOW() - INTERVAL 1 DAY) AND DATE(NOW() + INTERVAL 1 DAY) order by status DESC) m ON mi.match_id = m.match_id";
